...
<template>
  ...
  <div class="max-w-xl mx-auto text-black">
    <a href="http://lightcube.me/" class="flex justify-center px-10 py-2 mx-auto space-x-1 text-sm font-medium text-center text-gray-600 transition-all rounded-full shadow-sm group bg-white/30 ring-1 ring-gray-900/5 hover:shadow-lg active:shadow-sm">
      创建属于你的AI摇光
    </a>
      ...
      <div class="bg-white rounded-md shadow h-[60vh] flex flex-col justify-between">
        ...
        <form @submit.prevent="sendPrompt">
          <div class="flex items-center w-full p-4">
            <input v-model="message" type="text" placeholder="Type here..." class="w-full p-1 text-sm text-black bg-transparent bg-gray-100 border rounded-md shadow border-white/40 grow" />
            <button :disabled="loading" type="submit" class="flex items-center justify-center flex-none w-10 h-10 ml-2 bg-green-500 rounded-full">
    ...
    <div class="flex items-center justify-center my-2">
      <span>摇光人格</span>
      <a href="http://lightcube.me/" class="flex items-center mx-1 font-medium underline transition-colors underline-offset-4 hover:text-black/70">
        <p>LightCube.Me</p>
      </a>
      <button @click="clearHistory" class="btn btn-danger">清除历史记录</button>
    </div>
  </div>
</template>
